...
function! GitStatus()
    ...
    nnoremap <buffer> d       :silent !git diff <cfile><Enter><down>
    nnoremap <buffer> r       :call <SID>ResetFileInGitStatus()<Enter>
endfunction

function! s:ResetFileInGitStatus()
    let nr = input("Discard changes ? (y/n) ")
    if nr == 'y'
      execute ":silent !git checkout -- ".expand('<cfile>')
      execute ":call <SID>RefreshGitStatus()"
    endif
endfunction
